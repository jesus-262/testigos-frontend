...
refrescarusuario=async()=>{
  ...
  const tipo=this.token().tipo;
  console.log("refrescar usuario");
  console.log(id);
  console.log(tipo);
    var params={
  id:this.token().id,
 ...
 //console.log("this.state.navegacion")
  ...
  renderbotoneliminar= (usuario) => {
    ...
    if(this.token().tipo=="ADMINISTRADOR"){
    return <td><Link  to="/roles" className="btn btn-danger glyphicon glyphicon-trash" onClick={()=>this.borrar(usuario.id)}></Link></td>
  }
  ...
  renderbotonedictar= (usuario) => {
    if(this.token().tipo=="ADMINISTRADOR"){
    return <td><Link  to={"/roles/edictar/"+usuario.id} className="btn btn-warning glyphicon glyphicon-wrench" ></Link></td>
  }
  if(this.token().tipo=="GESTOR"){
    return <td><Link  to={"/roles/edictar/"+usuario.id} className="btn btn-warning glyphicon glyphicon-wrench" ></Link></td>
  }
